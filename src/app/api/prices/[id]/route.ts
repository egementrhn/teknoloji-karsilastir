...
async function fetchAmazonPrice(url: string): Promise<PriceResult> {
    try {
        ...
        return { store: 'Amazon', price: null, currency: '₺', url, updatedAt: new Date().toISOString() };
    }
}
...
async function fetchTrendyolPrice(url: string): Promise<PriceResult> {
    try {
        ...
        return { store: 'Trendyol', price: null, currency: '₺', url, updatedAt: new Date().toISOString() };
    }
}
...
async function fetchHepsiburadaPrice(url: string): Promise<PriceResult> {
    try {
        ...
        if (!response.ok) throw new Error(`Hepsiburada API error: ${response.status}`);
        const data = await response.json();
        ...
        return { store: 'Hepsiburada', price: null, currency: '₺', url, updatedAt: new Date().toISOString() };
    }
}
...
export async function GET(
    ...
    if (storeUrls.hepsiburada) fetches.push(fetchHepsiburadaPrice(storeUrls.hepsiburada));
