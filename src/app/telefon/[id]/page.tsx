...
export default async function PhoneDetail({ params }: PageProps) {
  ...
  const livePrices = await getLivePrices(id);
  const displayPrices: LivePrice[] =
      ...
      <section className={styles.productHeader}>
        ...
        <div className={styles.productInfo}>
          ...
          <p className={styles.releaseDate}>
            Çıkış Tarihi: {phone.releaseDate}
          </p>
            ...
            {lowestPrice ? (
              <div className={styles.priceHighlight}>
                ...
                <span className={styles.amount}>
                  {new Intl.NumberFormat("tr-TR").format(lowestPrice.price)}
                </span>
                <span className={styles.store}>
                  👉 {lowestPrice.store}&apos;da
                </span>
              </div>
            ) : (
            ...
            <table className={styles.specTable}>
              <tbody>
                <tr>
                  <th>Ekran</th>
                  <td>
                    {phone.specifications.Display?.Size || "N/A"}
                    <br />
                    <small>
                      {phone.specifications.Display?.Type?.split(",")[1] ||
                ...
                <tr>
                  ...
                  <td>
                    {phone.specifications.Memory?.Internal?.split(",")[0] ||
                      "N/A"}
                  </td>
                </tr>
                <tr>
                  <th>Kamera</th>
                  <td>
                    {phone.specifications?.["Main Camera"]?.Modules ||
                      phone.specifications?.["Main Camera"]?.Single ||
                      phone.specifications?.["Main Camera"]?.["Dual"] ||
                      phone.specifications?.["Main Camera"]?.["Triple"] ||
                      phone.specifications?.["Main Camera"]?.["Quad"] ||
                      "N/A"}
                    <br />
                    <small>
                      Ön:{" "}
                      {phone.specifications?.["Selfie camera"]?.Single ||
                        phone.specifications?.["Selfie camera"]?.Modules ||
                        "N/A"}
                    </small>
                  </td>
                </tr>
                ...
                {displayPrices.map((price, idx) => (
                  <div key={idx} className={styles.priceItem}>
                    ...
                    <div className={styles.priceValue}>
                      {new Intl.NumberFormat("tr-TR").format(price.price)}{" "}
                      {price.currency}
                    </div>
                    <a
                      href={price.url}
